...
from utils import get_embedding_weight, get_accuracy, return_accuracy
...
def select_subset_snli(dataset, data_type='entailment'):
    ...
    dataset_label_filter = data_type # 'entailment' # only entailment examples
...
if __name__ == "__main__":
    ...
    parser.add_argument('--attack_class', type=str, default='0',
    ...
    print(count, snli_vocab.get_vocab_size())
    ...
    target_data_type = 'contradiction'#args.snli_datatype
    targeted_dev_data = select_subset_snli(dev_dataset, data_type=target_data_type)

    model.get_metrics(reset=True)
    for batch in lazy_groups_of(iterator(targeted_dev_data, num_epochs=int(1), shuffle=True), group_size=1):
        batch_data = move_to_device(batch[0], cuda_device=0)
        SNLI_hypo_attack(model, batch_data['premise'], batch_data['hypothesis'], batch_data['label'], 0,[])
    print(model.get_metrics()['accuracy'])

    model.get_metrics(reset=True)
    get_accuracy(model, targeted_dev_data, snli_vocab, trigger_token_ids=None, snli=True)
    ...
    # initialize noise
    noise_n = 32  # this should be a factor of batch_size
    n_repeat = 1
    ...
    for tmp in range(tot_runs):
        ...
        for batch in lazy_groups_of(iterator(targeted_dev_data, num_epochs=int(5e5), shuffle=True), group_size=1):
            ...
            loss = output["loss"]
            if iter == 0:
                print(' initial loss value is ', loss.item())
                ...
                if update:
                    ...
                    for ss in sen_idxs:
                        ...
                        if not (s_str in sentence_list):
                            accuracy = return_accuracy(model, targeted_dev_data, snli_vocab, trigger_token_ids,
                                                       snli=True)
    ...
    select_fluent_trigger(all_output, GPT2_model, GPT2_tokenizer, 0.02)