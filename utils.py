...
def evaluate_batch(model, batch, trigger_token_ids=None, snli=False, append_mode='hypothesis'):
    """
...
def get_accuracy(model, dev_dataset, vocab, trigger_token_ids=None, snli=False, print_acc=True, thold=0.5):
    """
    ...
    if trigger_token_ids is None:
        ...
        if model.get_metrics()['accuracy'] < thold:
            if print_acc:
...
def return_accuracy(model, dev_dataset, vocab, trigger_token_ids=None, snli=False, append_mode='hypothesis'):
    """
    When trigger_token_ids is None, gets accuracy on the dev_dataset. Otherwise, gets accuracy with
    triggers prepended for the whole dev_dataset.
    """
    ...
    if trigger_token_ids is None:
        ...
        for batch in lazy_groups_of(iterator(dev_dataset, num_epochs=1, shuffle=False), group_size=1):
            evaluate_batch(model, batch, trigger_token_ids, snli, append_mode)
        #print("Current Triggers: " + print_string + " : " + str(model.get_metrics()['accuracy']))
    return model.get_metrics()['accuracy']
